...
/**
 * A Variable-Length Array with primitive long values; provides some additional methods that use polymorphism to avoid (un)boxing.
 ...
 */
public class LongVLA implements RandomAccess, Cloneable, java.io.Serializable, List<Long>, Comparable<List<? extends Long>> {
     ...
     */
    protected LongVLA(final long a[], boolean dummy) {
        this.a = a;
    }
    ...
    public LongVLA(final int capacity) {
     ...
     */
    public LongVLA(final LongVLA l) {
        this(l.size());
     ...
     */
    private LongVLA(final long a[], final int offset, final int length) {
        this(length);
     ...
     */
    public LongVLA(final Iterator<? extends Long> i) {
        this();
        while (i.hasNext())
            this.add(i.next());
    }

    public static LongVLA single(long item)
    {
        LongVLA lv = new LongVLA(1);
        lv.a[0] = item;
        return lv;
    }
     ...
     */
    private static LongVLA wrap(final long a[], final int length) {
        if (length > a.length)
            throw new IllegalArgumentException("The specified length (" + length + ") is greater than the array size (" + a.length + ")");
        final LongVLA l = new LongVLA(a, false);
        l.size = length;
        return l;
    }
     ...
     */
    public static LongVLA wrap(final long a[]) {
    ...
    private boolean rem(final long k) {
        int index = indexOf(k);
    ...
    public void size(final int size) {
        ...
        if (size > this.size) Arrays.fill(a, this.size, size, ((long) 0));
        this.size = size;
    }
     ...
     */
    private void getElements(final int from, final long[] a, final int offset, final int length) {
    ...
    public void removeElements(final int from, final int to) {
        LongArrays.ensureFromTo(size, from, to);
    ...
    public boolean addAll(final int index, final LongVLA l) {
        ensureIndex(index);
        final int n = l.size();
        if (n == 0) return false;
        grow(size + n);
    ...
    public class LongListIterator implements Iterator<Long>, ListIterator<Long> {

        public LongListIterator(int idx) {
            index = idx;
        }
        ...
        public void set(long k) {
            if (last == -1) throw new IllegalStateException();
            LongVLA.this.set(last, k);
        }

        public void remove() {
            if (last == -1) throw new IllegalStateException();
            LongVLA.this.removeLong(last);
    ...
    public List<Long> subList(int fromIndex, int toIndex) {
        return new LongVLA(a, fromIndex, toIndex - fromIndex);
    }

    public LongVLA clone() {
        LongVLA c = new LongVLA(size);
     ...
     */
    public boolean equals(final LongVLA l) {
     ...
     */
    public long compareTo(final LongVLA l) {
    ...
    /**
     * Compares this list to another object. If the argument is a {@link List}, this method performs a lexicographical comparison; otherwise, it throws a <code>ClassCastException</code>.
     *
     * @param l a list.
     * @return if the argument is a {@link List}, a negative integer, zero, or a positive integer as this list is lexicographically less than, equal to, or greater than the argument.
     * @throws ClassCastException if the argument is not a list.
     */

    public int compareTo(final List<? extends Long> l) {
        if (l == this) return 0;
        if (l instanceof LongVLA) {
            final LongListIterator i1 = listIterator(), i2 = ((LongVLA) l).listIterator();
            long r;
            long e1, e2;
            while (i1.hasNext() && i2.hasNext()) {
                e1 = i1.nextLong();
                e2 = i2.nextLong();
                if ((r = (e1 - e2)) != 0) return (int)r;
            }
    ...
    public <T> T[] toArray(T[] a) {
        Long[] b;
        if (a.length < size()) b = new Long[size()];
        else b = (Long[]) a;
    ...
    private static class LongArrays    /** A static, final, empty array. */
    {
