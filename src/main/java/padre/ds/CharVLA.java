...
/**
 * A Variable-Length Array with primitive char values; provides some additional methods that use polymorphism to avoid (un)boxing.
 ...
 */
public class CharVLA implements RandomAccess, Cloneable, java.io.Serializable, List<Character>, Comparable<List<? extends Character>> {
     ...
     */
    protected CharVLA(final char a[], boolean dummy) {
        this.a = a;
    }
    ...
    public CharVLA(final int capacity) {
     ...
     */
    public CharVLA(final CharVLA l) {
        this(l.size());
     ...
     */
    public CharVLA(final String s) { this(s.toCharArray());}
    /**
     ...
     */
    private CharVLA(final char a[], final int offset, final int length) {
        this(length);
     ...
     */
    public CharVLA(final Iterator<? extends Character> i) {
        this();
        while (i.hasNext())
            this.add(i.next());
    }
     ...
     */
    private static CharVLA wrap(final char a[], final int length) {
        if (length > a.length)
            throw new IllegalArgumentException("The specified length (" + length + ") is greater than the array size (" + a.length + ")");
        final CharVLA l = new CharVLA(a, false);
        l.size = length;
        return l;
    }
     ...
     */
    public static CharVLA wrap(final char a[]) {
    ...
    public boolean addAll(final int index, final CharVLA l) {
        ensureIndex(index);
        final int n = l.size();
        if (n == 0) return false;
        grow(size + n);
    ...
    public class CharListIterator implements Iterator<Character>, ListIterator<Character> {
        ...
        public void set(char k) {
            if (last == -1) throw new IllegalStateException();
            CharVLA.this.set(last, k);
        }

        public void remove() {
            if (last == -1) throw new IllegalStateException();
            CharVLA.this.removeChar(last);
    ...
    public List<Character> subList(int fromIndex, int toIndex) {
        return new CharVLA(a, fromIndex, toIndex - fromIndex);
    }

    public CharVLA clone() {
        CharVLA c = new CharVLA(size);
     ...
     */
    public boolean equals(final CharVLA l) {
    ...
    public int compareTo(final CharVLA l) {
    ...
    public int compareTo(final List<? extends Character> l) {
        if (l == this) return 0;
        if (l instanceof CharVLA) {
            final CharListIterator i1 = listIterator(), i2 = ((CharVLA) l).listIterator();
            int r;
            char e1, e2;
